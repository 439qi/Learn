...
Rust 中的所有变量都有固定的作用域，超出作用域之后，该变量就会被销毁  
变量在作用域中从初始化到销毁的整个过程称之为**生命周期**(lifetime)  
**引用**的生命周期表明该引用所指向变量的作用域范围  

通常情况下，生命周期都是隐式的，编译器会根据一些简单的规则自动推断  
复杂情况下，编译器无法准确的判断==引用==的生命周期，则需要手动标注  

生命周期标注不会改变实际的生命周期长度，其用于向编译器描述多个引用的生命周期之间的关系，如果不符合标注所约束的生命周期，编译器拒绝编译  
生命周期标注可以是任意合法的名称  
...
函数签名中的生命周期标注使用函数生命周期参数，其声明形式上类似于泛型声明  
此处标注指示编译器返回值的生命周期，小于具有相同标注的所有实参的生命周期中的最小值  

```rust
fn longer<'a>(x: &'a str, y: &'a str) -> &'a str { //do sth; }
```

> [!example]  
>
> 如下代码中，以实参 `xx` 和 `yy` 调用 `longer()` 时，编译器将 `xx` 和 `yy` 的生命周期与生命周期标注 `'a` 建立关联，取两者中生命周期较小者建立关联，此处即为 `yy`  
> 此处返回值绑定变量 `max` 的生命周期明显长于 `yy` 生命周期，因而编译失败  
>
> ```rust
...
函数存在多个生命周期参数时，需要标注各个生命周期参数之间的关系  

> [!example] 生命周期约束语法  
>
> `'b:'a` 表示 `'b` 的生命周期大于等于 `'a`  
>
> ```rust
...
> [!warning] 结构体中的引用字段必须添加生命周期标注  

一个包含引用成员的结构体，必须保证自身的生命周期不超过任何一个引用成员的生命周期  
...
1. 编译器为每一个引用参数分配不同的生命周期  
2. 如果只有一个输入生命周期，则其会被赋予所有输出生命周期  
3. 如果有多个输入生命周期且其中一个为 `&self` / `&mut self`，所有输出生命周期被赋予 `self` 的生命周期  

### 10.4 静态生命周期  

具有静态生命周期的引用存活于整个程序期间  
